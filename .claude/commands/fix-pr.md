...
For each chunk, launch a `general-purpose` subagent via the Task tool. All subagents should be launched in a **single message** so they run concurrently (foreground, not background — per CLAUDE.md sub-agent pattern).
...
3. **Clear instructions**: fix the issues described, follow CLAUDE.md guidelines (Svelte 5 runes, TypeScript strict, no stubs), and run `pnpm check` after making changes to verify correctness
...
- "After making changes, run `pnpm check` to verify no type errors were introduced."
- "Follow all CLAUDE.md guidelines — Svelte 5 runes, TypeScript strict mode, no `@ts-ignore`, no stubs."
- "Do NOT refactor beyond what the PR comment asks for. Make minimal, focused fixes."

### Step 4: Review Results

After all subagents complete:
1. Run `pnpm check` to verify the full project still type-checks
